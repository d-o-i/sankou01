...
// サウンドBGMファイルのパス（hの通しナンバーと合わせること）
const TCHAR* c_soundBGMFilename[] = {
	_T("data/BGM/snowdrop.wav"),
	_T("data/BGM/super nova.wav"),
	_T("data/BGM/deeper.wav"),
	_T("data/BGM/Daydream.wav"),
	_T("data/BGM/sample000.wav"),
};

// サウンドSEファイルのパス（hの通しナンバーと合わせること）
const TCHAR* c_soundSEFilename[] = {
	_T("data/SE/tap.wav"),
	_T("data/SE/tamb just..wav"),
	_T("data/SE/tamb good.wav"),
};

// グローバル変数
IDirectSound8 *g_pDirectSound = NULL;	// サウンドインターフェース
IDirectSound8 *g_pPerformance;
...
HRESULT InitSound(HWND hWnd)
{
	...
	//wFmt.nBlockAlign = wFmt.nChannels * wFmt.wBitsPerSample / 12;
...
LPDIRECTSOUNDBUFFER8 LoadSoundBGM(int no)
{
	// MMIO = マルチメディア入出力、の略。
	...
	hMmio = mmioOpen((LPSTR)c_soundBGMFilename[no], &mmioInfo, MMIO_READ);
	if (!hMmio)
		return NULL;
	...
	//g_pDirectSoundB->CreateSoundBuffer(&DSBufferDesc, &ptmpBuf, NULL);
...
LPDIRECTSOUNDBUFFER8 LoadSoundSE(int no)
{
	// MMIO = マルチメディア入出力、の略。
	LPDIRECTSOUNDBUFFER  pBaseBuffer = NULL;// 曲データの総合バッファ
	LPDIRECTSOUNDBUFFER8 pBuffer = NULL;	// 曲データのバッファ
	...
	DWORD  dwSize;				// セカンダリバッファのサイズ
								...
								// 1.ハンドルをもとにファイルを開く
	memset(&mmioInfo, 0, sizeof(MMIOINFO));
	hMmio = mmioOpen((LPSTR)c_soundSEFilename[no], &mmioInfo, MMIO_READ);
	if (!hMmio)
		return NULL;
	...
	//IDirectSoundBuffer *ptmpBuf = 0;
	//IDirectSoundBuffer8 *pDSBuffer;
	//g_pDirectSound->CreateSoundBuffer(&DSBufferDesc, &ptmpBuf, NULL);
	//ptmpBuf->QueryInterface(IID_IDirectSoundBuffer8, (void**)&pDSBuffer);
	//ptmpBuf->Release();
...
void PlaySound(LPDIRECTSOUNDBUFFER8 pBuffer, int flag)
